...
def render_markdown(name, template_folder, template_file, **data_obj):
    ...
    :param template_file
    """
    env = jinja2.Environment(loader=jinja2.PackageLoader(name, template_folder))
...
def sort_app_data(data_obj, order:list=None):
    ...
    priority_max = len(priority_dict)+ START+1
    result_sort = sorted(data_obj, key=lambda e:priority_dict.get(e.get('name')) or priority_max)
    data_obj = sorted(result_sort, key=lambda e:priority_dict.get(e.get('group')) or priority_max)
